...
::: {.callout-note}
## Interpretación
El método de momentos recupera estimaciones muy próximas a los parámetros poblacionales verdaderos de la glucosa basal (μ̂ ≈ 95 mg/dL vs μ = 95; σ̂² ≈ 144 vs σ² = 144), evidenciando que el procedimiento de igualar momentos muestrales con poblacionales proporciona estimadores consistentes. Esta propiedad es fundamental en estudios epidemiológicos sobre control glucémico, donde se busca estimar parámetros poblacionales de biomarcadores (glucemia, HbA1c, insulina) a partir de cohortes muestrales. La proximidad de las estimaciones al valor verdadero con n = 100 pacientes valida el MoM como método práctico y computacionalmente eficiente para caracterizar distribuciones de variables clínicas continuas.
:::

:::

### Ejemplo 6.5.3: Distribución de Poisson
...
### Ejemplo 6.5.4: Distribución Gamma
...
**Nota:** Para la media, el EMV coincide con el estimador MoM. Para la varianza, el EMV es $S'^2$ (con divisor $n$), que es sesgado, mientras que el EMV del MoM es también $S'^2$.
...
**Interpretación:** Con el 95% de confianza, el nivel medio de HbA1c poblacional en pacientes diabéticos está entre 7.54% y 8.62%. Este intervalo incluye el objetivo clínico de HbA1c < 7%, sugiriendo que la cohorte presenta control glucémico moderado. El margen de error (~0.54%) refleja la variabilidad en control metabólico entre pacientes, lo que en práctica clínica orienta decisiones de intensificación terapéutica.
:::
...
::: {.callout-tip}
## Ejemplo con BioestadisticaR2: icm() e icp()

Documentación completa de ambas funciones en @sec-bio-ic.

```{r}
#| lst-label: lst-semana07-7
#| lst-cap: "Código R"
#| warning: false
#| message: false
...
`icp()` calcula cuatro métodos simultáneamente. Para muestras pequeñas se recomienda Clopper-Pearson (exacto); para muestras grandes los cuatro coinciden.
:::

## Cálculo del Tamaño Muestral: `nm()` y `np()` {#sec-semana06-tamano}

Antes de iniciar un estudio clínico es esencial determinar el **tamaño muestral** necesario para alcanzar una precisión deseada en los intervalos de confianza. Las funciones `nm()` y `np()` del paquete `BioestadisticaR2` (ver @sec-bio-n) calculan el tamaño muestral requerido para una **media** o una **proporción**, respectivamente, dada una semiamplitud objetivo $d$ del IC.
...
::: {.callout-note}
## Interpretación
El cálculo del tamaño muestral cuantifica el coste estadístico de incrementar la precisión. Para estimar la HbA1c media con una semiamplitud del IC $\leq 0.3\%$ (frente a los $\approx 0.37\%$ obtenidos con $n = 94$), se necesitarían aproximadamente **n ≈ 138 pacientes diabéticos** — un incremento del 47%. Análogamente, para estimar la prevalencia de tabaquismo con precisión $\pm 5\%$ (IC 95%) se requerirían cerca de **n ≈ 380 pacientes**, cuatro veces el tamaño del estudio piloto. En la fase de diseño de un protocolo clínico, estos cálculos orientan el balance entre **precisión científica** (estimaciones más estrechas guían mejor las decisiones terapéuticas) y **viabilidad logística-financiera** (reclutamiento, seguimiento, presupuesto). La dependencia cuadrática $n \propto 1/d^2$ explica por qué ganar un factor 2 en precisión cuadruplica el coste muestral — un argumento decisivo al justificar el tamaño muestral ante comités éticos y agencias financiadoras.
:::
:::
...
IC99%: más amplio ≈ [44.35, 55.65] (usa t₀.₀₀₅,₃₅ ≈ 2.72)
