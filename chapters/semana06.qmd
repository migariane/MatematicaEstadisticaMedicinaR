...
::: {.callout-tip}
## Ejercicio 6.1: Concepto de Estimador

(a) Explique, con palabras propias, la diferencia entre un parámetro poblacional, un estimador y un valor estimado.
(b) ¿Por qué es importante considerar un estimador como una variable aleatoria?
:::

::: {.callout-tip}
## Ejercicio 6.2: Insesgadez vs. Eficiencia

Considere dos estimadores insesgados del mismo parámetro, uno con el doble de varianza que el otro.

(a) Indique cuál resulta preferible.
(b) Justifique la respuesta.
:::

::: {.callout-tip}
## Ejercicio 6.3: Error Cuadrático Medio

(a) Explique por qué un estimador sesgado con baja varianza puede ser preferible a uno insesgado con alta varianza.
(b) Incluya un ejemplo numérico simple.
:::

::: {.callout-tip}
## Ejercicio 6.4: Método de Momentos

Para una muestra de una distribución $\operatorname{Uniforme}(0,\theta)$:

(a) Obtenga el estimador de $\theta$ por el método de momentos.
(b) Determine si el estimador es insesgado.
:::

::: {.callout-tip}
## Ejercicio 6.5: Máxima Verosimilitud

Para una muestra de $n$ observaciones de una distribución $\operatorname{Poisson}(\lambda)$:

(a) Escriba la función de verosimilitud.
(b) Obtenga el EMV de $\lambda$.
(c) Indique si coincide con el estimador por el método de momentos.
:::

::: {.callout-tip}
## Ejercicio 6.6: Intervalo de Confianza

Se observa una muestra de 36 datos con $\bar{x} = 50$ y $s = 12$.

(a) Construya un IC del 95% para la media poblacional.
(b) ¿Cómo cambiaría el intervalo con un nivel de confianza del 99%?
:::

::: {.callout-tip}
## Ejercicio 6.7: Intervalo de Confianza para Proporción

En una encuesta de 500 personas, 325 responden afirmativamente a una pregunta.

(a) Construya un IC del 95% para la proporción poblacional.
(b) Interprete el resultado.
:::

::: {.callout-tip}
## Ejercicio 6.8: Programación en R

Escriba un script en R que:

(a) Genere una muestra de una distribución normal con $\mu=100$ y $\sigma=15$, de tamaño $n=50$.
(b) Calcule estimadores puntuales para $\mu$ y $\sigma^2$.
(c) Construya un IC del 95% para $\mu$.
(d) Verifique si el verdadero valor de $\mu$ queda dentro del intervalo.
:::
...
(a) Un parámetro poblacional es un valor fijo, generalmente desconocido, que describe una característica de la población; un estimador es una función de las variables muestrales, $T(X_1,\ldots,X_n)$; y un valor estimado es el resultado numérico concreto, $T(x_1,\ldots,x_n)$, obtenido después de observar una muestra.
(b) El estimador depende de la muestra seleccionada y, por tanto, es una variable aleatoria. Esto permite estudiar su distribución, sesgo y varianza.

**Ejercicio 6.2:**

(a) Se prefiere el estimador con menor varianza.
(b) Al ser ambos insesgados, el estimador con menor varianza también tiene menor error cuadrático medio y mayor eficiencia.

**Ejercicio 6.3:**

(a) $\operatorname{ECM}(\hat{\theta})=\operatorname{Var}(\hat{\theta})+\operatorname{Sesgo}(\hat{\theta})^2$; una reducción suficiente de la varianza puede compensar un sesgo pequeño.
(b) Para $\theta=0$, A tiene sesgo 0 y varianza 100, mientras que B tiene sesgo 2 y varianza 10: $\operatorname{ECM}(A)=100$ y $\operatorname{ECM}(B)=14$. Por tanto, B presenta menor ECM a pesar de ser sesgado.

**Ejercicio 6.4:**

(a) $\hat{\theta}_{\mathrm{MoM}}=2\bar{X}$.
(b) $E(\hat{\theta}_{\mathrm{MoM}})=2E(\bar{X})=\theta$; por tanto, es insesgado.
...
(a) La función de verosimilitud es
    ...
    {\prod_{i=1}^{n}x_i!}.
    $$
(b) $\hat{\lambda}_{\mathrm{EMV}}=\frac{1}{n}\sum_{i=1}^{n}x_i=\bar{X}$.
(c) $\hat{\lambda}_{\mathrm{MoM}}=\bar{X}$; ambos estimadores coinciden.

**Ejercicio 6.6:**

(a) $SE=s/\sqrt{n}=12/\sqrt{36}=2$; por tanto, $IC_{95\%}=50\pm t_{0.975,35}(2)\approx50\pm4.060=[45.94,54.06]$.
(b) $IC_{99\%}=50\pm t_{0.995,35}(2)\approx50\pm5.448=[44.55,55.45]$; es más amplio por el mayor nivel de confianza.

**Ejercicio 6.7:**

(a) $\hat{p}=325/500=0.65$, $SE(\hat{p})\approx0.0213$ e $IC_{95\%}=0.65\pm1.96(0.0213)\approx[0.608,0.692]$.
(b) La proporción poblacional de respuestas afirmativas se estima entre aproximadamente el 60.8% y el 69.2% mediante un procedimiento con un nivel de confianza del 95%.

**Ejercicio 6.8:**

Al estimarse la varianza con la muestra, el intervalo para $\mu$ utiliza la distribución t de Student:
...
# (a)
muestra <- rnorm(n, mean = mu_real, sd = 15)
...
ic <- media_est + c(-1, 1) * qt(0.975, n - 1) * se

# (d)
contiene_mu <- ic[1] <= mu_real && mu_real <= ic[2]

c(media_est = media_est, var_est = var_est)
ic
contiene_mu
```

(a) Se genera la muestra normal de tamaño 50 en `muestra`.
(b) $\hat{\mu}\approx100.516$ y $\hat{\sigma}^2\approx192.878$.
(c) $IC_{95\%}(\mu)\approx[96.569,104.463]$.
(d) `contiene_mu = TRUE`; el intervalo contiene el valor $\mu=100$.
