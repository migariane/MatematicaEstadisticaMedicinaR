...
### Z-scores con Parámetros Poblacionales {#sec-semana02-s3-ejemplo-pob}
...
#| lst-cap: "Covarianza y correlación entre peso e IMC en osteo"
...
- La **covarianza** es positiva ($\widehat{\text{Cov}}(\text{peso}, \text{IMC}) > 0$), lo que indica que peso e IMC tienden a crecer simultáneamente. Sin embargo, su magnitud depende de las unidades y no es directamente comparable entre estudios.
- La **correlación de Pearson** $r \approx 0.7$ con $p < 0.001$ cuantifica la asociación de forma adimensional: existe una asociación lineal positiva fuerte. Esto es **esperable por construcción** ($\text{IMC} = \text{peso} / \text{talla}^2$), pero la dispersión visible en el gráfico refleja la heterogeneidad debida a la talla.
...
tema_alto_contraste <- theme_minimal(base_size = 12) +
  theme(plot.title = element_text(face = "bold"),
        panel.grid.minor = element_blank())

p1 <- ggplot(df1, aes(x, y)) +
  geom_point(color = "#0072B2", size = 1.8, alpha = 0.85) +
  labs(title = paste("No lineal, r =", r1)) + tema_alto_contraste
p2 <- ggplot(df2, aes(x, y)) +
  geom_point(color = "#D55E00", size = 1.8, alpha = 0.85) +
  labs(title = paste("Círculo, r =", r2)) + tema_alto_contraste
p3 <- ggplot(df3, aes(x, y)) +
  geom_point(color = "#009E73", size = 1.8, alpha = 0.85) +
  labs(title = paste("Lineal, r =", r3)) + tema_alto_contraste
