...
- **Por filas** ($P(\text{Infección} \mid \text{Test})$): **Valor Predictivo** — responde a la pregunta clínica crucial: "Dado que el paciente tiene este resultado de test, ¿cuál es la probabilidad real de que esté infectado?".
- **Por columnas** ($P(\text{Test} \mid \text{Infección})$): **Sensibilidad y Especificidad** — qué tan bueno es el test para identificar la presencia o ausencia de enfermedad.
:::
...
En ambas escalas, una puntuación más alta indica mayor deterioro.
...
Se miden presión sistólica ($X$) y diastólica ($Y$) en una muestra clínica. Se encuentra que $\operatorname{Cov}(X,Y)=180$ $(\text{mmHg})^2$, $s_X=15$ mmHg y $s_Y=10$ mmHg.

(a) Calcula la correlación entre $X$ e $Y$.
...
- **(a)** Rango: $320-240=80$ ms. IQR: $Q_3-Q_1=292.5-258.75=33.75$ ms (`quantile()` por defecto, `type = 7`).
- **(b)** Media: 276.25 ms. Desviación estándar: $\approx24.69$ ms. Mediana: 272.5 ms.
- **(c)** Límites: $208.125$ y $343.125$ ms. No se detectan valores atípicos.
- **(d)** El diagrama de caja muestra una distribución aproximadamente simétrica alrededor de 275-280 ms, sin valores atípicos.

**Ejercicio 2.2: Transformaciones Lineales (Conversión)**

- **(a)** $\bar{y}=1.36(15)=20.4$ cmH₂O.
- **(b)** $s_y=1.36(3)=4.08$ cmH₂O.
- **(c)** $\operatorname{IQR}_y=1.36(4)=5.44$ cmH₂O.

**Ejercicio 2.3: Z-Scores y Estandarización Clínica**

- **(a)** Escala A: $z_A=(85-75)/10=1.00$. Escala B: $z_B=(90-80)/15\approx0.67$.
- **(b)** El deterioro relativo es mayor en la escala A, porque $z_A>z_B$.
- **(c)** El umbral $z>0.5$ se supera en ambas escalas.

**Ejercicio 2.4: Tablas de Contingencia**

- **(a)** Fuman: $110/300=0.367$ (36.7%); no fuman: $190/300=0.633$ (63.3%).
- **(b)** Entre quienes fuman: problemas, $70/110=0.636$ (63.6%); sin problemas, $40/110=0.364$ (36.4%).
- **(c)** Entre quienes no fuman: problemas, $30/190=0.158$ (15.8%); sin problemas, $160/190=0.842$ (84.2%).
- **(d)** La proporción observada de problemas respiratorios es 47.8 puntos porcentuales mayor entre quienes fuman y equivale aproximadamente a 4.03 veces la observada entre quienes no fuman. Esto describe una asociación, no una relación causal.

**Ejercicio 2.5: Análisis de Datos Clínicos Simulados**

- **(a)** Edad: media 48.3921 años y desviación estándar 14.1486 años. Presión sistólica: media 128.3536 mmHg y desviación estándar 10.5171 mmHg. Covarianza: 85.4006 años·mmHg.
- **(b)** $r=0.5739$, lo que indica una asociación lineal positiva moderada.
...
- **(f)** Las medias de ambas variables estandarizadas son aproximadamente 0 y sus desviaciones estándar son 1.

**Ejercicio 2.6: Distribuciones Conjuntas**

- **(a)** El cálculo formal produce $r=180/[15(10)]=1.2$, un valor imposible. Los datos son incompatibles porque $180>15(10)=150$.
- **(b)** El valor obtenido no admite una interpretación estadística válida; debe revisarse al menos uno de los datos proporcionados.
- **(c)** No puede determinarse la independencia con datos mutuamente incompatibles.
