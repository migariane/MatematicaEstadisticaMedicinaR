...
freq_rel <- tabla_freq / length(pas_diabeticos)
freq_acum <- cumsum(tabla_freq)
...
::: {.callout-tip}
## Ejemplo 1.4: BioestadisticaR2: freq()
...
El 52.1% de los pacientes tiene menos de 28 años. Con `grf = TRUE` (por defecto) también genera el histograma. Con `cuts = 5` agrupa en 5 clases de igual amplitud.
:::
...
Cuando solo disponemos de datos agrupados en clases, aproximamos la media usando la marca de clase:

$$\bar{x} = \frac{1}{n}\sum_{j=1}^{k} x_j \cdot n_j$$

donde $x_j$ es la **marca de clase** (punto medio del intervalo $j$):
$$x_j = \frac{x_j^l + x_j^u}{2}$$

y $n_j$ es el número de observaciones en la clase $j$.
...
**Conjunto 1** (n impar): 115, 118, 120, 122, 124, 125, 128, 129, 130, 135
...
Mide el grado de concentración de los valores alrededor de la zona central de la distribución y el "peso" de las colas. Se calcula mediante el coeficiente de curtosis de Fisher:

$$g_2 = \frac{\frac{1}{n} \sum_{i=1}^{n} (x_i - \bar{x})^4}{s^4}$$

En muchos paquetes estadísticos (incluido R), se utiliza la **curtosis en exceso** ($g_2 - 3$) para comparar con la distribución normal.
...
Con los datos de supervivencia de pacientes (Ejemplo 1.6), podemos aproximar los cuartiles usando interpolación en la ECDF:
...
Considera el histograma de IMC del Ejemplo 1.5.1:
...
a) Media ≈ 101.4 mg/dL. b) Clase modal: 100-110 mg/dL. c) P₅₀ ≈ 100 + (0.50 - 0.47)/0.35 * 10 ≈ 100.86 mg/dL.

**Ejercicio 1.6: IMC (Ejemplo 1.5.1)**
