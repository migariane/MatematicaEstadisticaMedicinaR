...
::: {.callout-tip}
## Ejercicio 7.1: Conceptos Básicos

Un médico sospecha que el nivel medio de colesterol en una población es mayor que 200 mg/dL.

(a) Formule las hipótesis apropiadas.
(b) Identifique los errores de tipo I y de tipo II que podrían ocurrir.
:::

::: {.callout-tip}
## Ejercicio 7.2: Contraste Z

El diámetro de los tornillos producidos por una máquina tiene una desviación típica poblacional conocida de $\sigma=0.5$ mm. Se toma una muestra de 25 tornillos y se obtiene un diámetro medio de $\bar{x}=10.2$ mm. El diámetro especificado es 10 mm.

(a) Formule $H_0$ y $H_1$.
(b) Calcule el estadístico Z.
(c) Determine si se rechaza $H_0$ con $\alpha=0.05$ en un contraste bilateral.
(d) Calcule el p-valor.
:::

::: {.callout-tip}
## Ejercicio 7.3: Contraste t
...
(a) Calcule $\bar{x}$ y $s$.
(b) Calcule el estadístico t para contrastar $\mu=0.40$.
(c) Determine el p-valor bilateral.
(d) Determine si el tiempo medio difiere de 0.40 segundos con $\alpha=0.05$.

**Código R sugerido:**

```{r}
#| lst-label: lst-semana07-ejercicio3
#| lst-cap: "Código R"
#| warning: false
#| message: false
#| output: false
...
::: {.callout-tip}
## Ejercicio 7.4: Contraste Binomial

Un casino afirma que la probabilidad de obtener un 6 con uno de sus dados es $1/6$. Se lanza el dado 20 veces y se obtiene un 6 en 6 ocasiones.

(a) Formule $H_0$ y $H_1$.
(b) Calcule el p-valor bilateral exacto.
(c) Determine si se rechaza $H_0$ con $\alpha=0.05$.

**Código R sugerido:**

```{r}
#| lst-label: lst-semana07-ejercicio4
#| lst-cap: "Código R"
#| warning: false
#| message: false
#| output: false
...
::: {.callout-tip}
## Ejercicio 7.5: Decisión con p-valor

Para cada uno de los siguientes p-valores, indique si se rechaza $H_0$ con $\alpha=0.05$:
...
::: {.callout-tip}
## Ejercicio 7.6: Interpretación de Resultados

Un estudio investiga si el peso medio de una población difiere de 70 kg. Se obtiene $t=1.85$ con 24 grados de libertad y un p-valor de 0.077.

(a) ¿Se rechaza $H_0$ con $\alpha=0.05$?
(b) ¿Se rechazaría $H_0$ con $\alpha=0.10$?
(c) ¿Qué puede concluirse de estos resultados?
...
(a) $H_0:\mu\leq200$ mg/dL frente a $H_1:\mu>200$ mg/dL.
(b) Error de tipo I: concluir que $\mu>200$ cuando $\mu\leq200$. Error de tipo II: no detectar que $\mu>200$ cuando realmente $\mu>200$.

**Ejercicio 7.2:**

(a) $H_0:\mu=10$ mm frente a $H_1:\mu\neq10$ mm.
(b) $Z=(10.2-10)/(0.5/\sqrt{25})=2.0$.
(c) Los valores críticos son $\pm z_{0.975}=\pm1.96$; como $|Z|=2.0>1.96$, se rechaza $H_0$.
(d) $p=2P(Z>2.0)\approx0.0455$.

**Ejercicio 7.3:**

(a) $\bar{x}=0.415$ s y $s\approx0.0303$ s.
(b) $t=(0.415-0.40)/(0.0302765/\sqrt{10})\approx1.5667$, con 9 grados de libertad.
(c) El p-valor bilateral es aproximadamente 0.1516.
(d) Como el p-valor es $0.1516>0.05$, no se rechaza $H_0$; no hay evidencia suficiente para concluir que el tiempo medio difiera de 0.40 segundos.

**Ejercicio 7.4:**

(a) $H_0:p=1/6$ frente a $H_1:p\neq1/6$.
(b) Con $X\sim\operatorname{Binomial}(20,1/6)$, el p-valor bilateral exacto es aproximadamente 0.1279.
(c) Como el p-valor es $0.1279>0.05$, no se rechaza $H_0$; no hay evidencia suficiente para concluir que la probabilidad de obtener un 6 difiera de $1/6$.

**Ejercicio 7.5:**

(a) Como $0.031<0.05$, se rechaza $H_0$.
(b) Como $0.052>0.05$, no se rechaza $H_0$.
(c) Como $0.0001<0.05$, se rechaza $H_0$.
(d) Como $0.450>0.05$, no se rechaza $H_0$.

**Ejercicio 7.6:**

(a) Como $0.077>0.05$, no se rechaza $H_0$.
(b) Como $0.077<0.10$, se rechazaría $H_0$ con $\alpha=0.10$.
(c) La conclusión depende del nivel fijado previamente: no se rechaza $H_0$ al 5% y se rechazaría al 10%.
(d) No rechazar $H_0$ solo indica que no hay evidencia suficiente en su contra; no demuestra que sea verdadera.
