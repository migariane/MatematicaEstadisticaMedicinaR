...
Un hospital desea estimar el peso medio al nacer de los neonatos en una región. De una población de 5000 nacimientos anuales, selecciona mediante muestreo aleatorio simple sin reemplazo una muestra de 100 neonatos. Se sabe que la desviación típica poblacional es $\sigma = 200$ gramos.
...
Un centro de salud tiene 8,000 pensionistas a su cargo. Se desea estimar el gasto mensual medio por paciente en tratamientos anticoagulantes y protectores cardiovasculares avanzados. Se planea seleccionar mediante muestreo aleatorio simple sin reemplazo una muestra de 320 pacientes. Sabiendo que el gasto medio estimado ronda los 65 euros y la desviación típica poblacional es de $\sigma = 45$ euros:
...
(d) ¿En qué porcentaje se reduce la varianza al aplicar la CPF, tomando como referencia la varianza sin corrección?
:::
...
(c) Usando el error estándar obtenido en el apartado (b), construya un intervalo normal aproximado del 95% centrado en $\hat{\pi}$.
(d) Según el intervalo obtenido en el apartado (c), ¿es compatible un apoyo poblacional del 50% con el sondeo observado?
:::
...
Se mide el contenido de grasa en leche de 25 muestras. La varianza muestral es $s^2 = 1.44$ (porcentaje al cuadrado). Asuma que el contenido sigue una distribución normal y tome $\sigma^2 = 1.0$ como valor de referencia.
...
(d) Al nivel de significación del 5%, ¿hay evidencia de que la varianza poblacional sea mayor que $1.0$? Justifique su respuesta utilizando el resultado del apartado (c).
:::
...
- **(a)** $f = 320/8000 = 0.04 = 4\%$.
- **(b)** Como $f = 0.04 < 0.05$, según la regla práctica no es imprescindible aplicar la CPF. No obstante, puede utilizarse para obtener el valor exacto bajo muestreo sin reemplazo.
- **(c)** Sin CPF:

  $$
  \text{Var}(\bar{X}) = \frac{45^2}{320} \approx 6.33\ \text{€}^2.
  $$

  Con CPF:

  $$
  \text{Var}(\bar{X}) = \frac{45^2}{320} \times \frac{8000-320}{8000-1} \approx 6.08\ \text{€}^2.
  $$

- **(d)** La CPF reduce la varianza aproximadamente un $3.99\%$ respecto de la varianza sin corrección.

**Ejercicio 5.3:**

- **(a)** $\hat{\pi} = 324/600 = 0.54$.
- **(b)** $\sigma(\hat{\pi}) = \sqrt{0.5 \times 0.5 / 600} \approx 0.0204$.
- **(c)** Usando el error estándar de (b):

  $$
  0.54 \pm 1.96 \sqrt{\frac{0.5 \times 0.5}{600}} \approx [0.49999, 0.58001].
  $$

  Redondeado a tres decimales, el intervalo es $[0.500, 0.580]$.

- **(d)** Sí. Bajo esta aproximación, $0.50$ queda dentro del intervalo, aunque prácticamente sobre su límite inferior.
...
- **(d)** Bajo la aproximación normal, se necesitan al menos $1537$ trabajadores; es decir, $n \geq 1537$.
...
- **(d)** Como $p = 0.0752 > 0.05$, no hay evidencia suficiente para concluir que la varianza poblacional sea mayor que $1.0$.
